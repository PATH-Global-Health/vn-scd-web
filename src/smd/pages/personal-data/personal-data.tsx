...
import PersonalDataTable from '@smd/components/personal-data-table';
import PersonalDataHistoryTable from '@smd/components/personal-data-table/personal-data-history-table';
...
const PersonalDataPage: React.FC = () => {
  ...
  const sections = useMemo((): BreadcrumbSectionProps[] => {
    const bc: BreadcrumbSectionProps[] = [
      {
        key: 0,
        content: !selecting ? t('Individual data management') : t('Back'),
        active: !selecting,
  ...
  return (
    <>
      ...
      {!selecting && <PersonalDataTable onSelect={setSelecting} />}
      {selecting && <PersonalDataHistoryTable data={selecting} />}
    </>
  );
};
