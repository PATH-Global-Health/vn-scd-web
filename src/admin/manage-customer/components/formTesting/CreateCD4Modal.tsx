...
const CreateCD4Modal: React.FC<Props> = (props) => {
  ...
  const onSubmit = async (data: ResultTesting) => {
    try {
      ...
      toast(
        <ToastComponent
          content={t('Successful CD4 test update')}
          type="success"
        />,
      );
    } catch (error) {
      toast(
        <ToastComponent content={t('Failed CD4 test update')} type="failed" />,
      );
    }
  };

  return (
    <Modal open={Boolean(formType === 3)} onClose={onClose}>
      <Modal.Content>
        <Header as="h3">{t('CD4 test')}</Header>
          ...
          <Form.Field>
            ...
            <input
              ...
              ref={register({
                required: {
                  value: true,
                  message: t('Examination code not enterd'),
                },
              })}
            />
            {errors.code && (
